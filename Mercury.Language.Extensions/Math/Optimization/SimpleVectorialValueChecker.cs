...
{
    /// <summary>
    /// Simple implementation of the {@link IVectorialConvergenceChecker} interface using
    ...
    /// @since 2.0
    /// </summary>
    public class SimpleVectorialValueChecker : IVectorialConvergenceChecker
    {
        ...
        {
            double[] p = previous.PointReference;
            double[] c = current.PointReference;
            for (int i = 0; i < p.Length; ++i)
            {
                double pi = p[i];
                double ci = c[i];
                double difference = System.Math.Abs(pi - ci);
                double size = System.Math.Max(System.Math.Abs(pi), System.Math.Abs(ci));
