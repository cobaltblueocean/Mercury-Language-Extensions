...
namespace Mercury.Language.Math.Optimizer
{
    ...
    public class BaseMultivariateMultiStartOptimizer<F> : IBaseMultivariateOptimizer<F> where F : IMultivariateRealFunction
    {
        ...
        public Tuple<double[], double> Optimize(F f, GoalType goalType, double[] startPoint)
        {
            ...
            for (int i = 0; i < starts; ++i)
            {
                ...
                try
                {
                    optima[i] = optimizer.Optimize(f, goalType, i == 0 ? startPoint : generator.NextVector(startPoint.Length));
                }
